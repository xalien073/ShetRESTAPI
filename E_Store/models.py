...
class Product(models.Model):
    ...
    old_price = models.PositiveIntegerField()#DecimalField(max_digits=30, decimal_places=2)
    price = models.PositiveIntegerField()#DecimalField(max_digits=30, decimal_places=2)
...
def update_ratings(product):
    ...
    UpdatedRatings = (5*(star5) + 4*(star4) + 3*(star3) + 2*(star2) + 1*(star1))/tnr
    UpdatedRatings = str(UpdatedRatings)
    if float(UpdatedRatings) >= 5:
        UpdatedRatings = 5.0
    Product.objects.filter(product_id=product).update(Ratings=UpdatedRatings, Total_Reviews=tnr)    
